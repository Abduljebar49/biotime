...
export class ReportingService {
  static async getTodayReport(): Promise<any> {
    const sql = `
    WITH total AS (
      SELECT COUNT(*)::int AS total_employees FROM personnel_employee WHERE is_active = true
    ),
    present AS (
      SELECT 
        e.id, 
  ...
  static async getMonthlyReport(): Promise<any> {
    const sql = `
      SELECT 
        ...
        COUNT(CASE WHEN att.clock_in IS NOT NULL THEN 1 END) as total_present,
        COUNT(CASE WHEN att.clock_in IS NULL THEN 1 END) as total_absent,
        COUNT(CASE WHEN att.clock_in::time > '09:00:00' THEN 1 END) as total_late,
        COUNT(CASE WHEN att.clock_out IS NOT NULL AND att.clock_out::time < '17:00:00' THEN 1 END) as total_early_leave,
        COUNT(CASE WHEN EXTRACT(EPOCH FROM (att.clock_out - att.clock_in))/3600 > 8 THEN 1 END) as total_overtime,
